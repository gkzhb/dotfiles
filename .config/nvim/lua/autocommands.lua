...
function _G.fugitive_commit_foldexpr(lnum)
  ...
  local is_header = line:match('^tree ')
    or line:match('^parent ')
    or line:match('^author ')
    or line:match('^committer ')
  local is_header_continuation = prev:match('^gpgsig ')
    or prev:match('^ ')
    or line:match('^gpgsig ')
    or line:match('^ ')
